...
def runSinglePointVBS_sign(path_, model, categories, method, runSingleCat, years, UseHybridNew, unblind):
    ...
    if(os.path.exists(path)):
        ...
        if UseHybridNew:
            for idhn in range(5):
                runCombine("combine -M HybridNew " + modelname + "_" + method + ".txt --LHCmode LHC-significance --saveToys --saveHybridResult --fullBToys " + extraoption + " -T 50 -i 40 -s -1 --fork 10 -H AsymptoticLimits --rMin -4 -n " + modelname + "_hybrid", "hybrid_" + modelname + "_" + method + "_" + str(idhn) + ".log")
