...
//sprintf(path1,"likelihoodScan/%s/mjj/LS_objects_k_%s.root",argv[1],argv[1]);
 sprintf(path0,"./fit_v%s_DNN_%s_UL010_allBKG_countings_RunII_/%s/LS_objects_k_%s.root",argv[2],argv[1],argv[1],argv[1]);
                       
TFile *f_0 = TFile::Open(path0);
gr= (TGraph*) f_0->Get("Graph;1");
TF1 *f0 = new TF1("f0",myfunc,-1000,1000,0);

double s1down_0=f0->GetX(y2,-1000,0);
double s1up_0=f0->GetX(y2,0,1000);
double s2down_0=f0->GetX(y1,-1000,0);
double s2up_0=f0->GetX(y1,0,1000);

double min_0=f0->GetMinimumX(-10,10);
...
//sprintf(path1,"likelihoodScan/%s/mjj/LS_objects_k_%s.root",argv[1],argv[1]);
 sprintf(path1,"./fit_v%s_DNN_%s_UL010_allBKG_DNN_%s_UL010_allBKG_RunII_/%s/LS_objects_k_%s.root",argv[2],argv[1],argv[1],argv[1],argv[1]);
                       
TFile *f_1 = TFile::Open(path1);
gr= (TGraph*) f_1->Get("Graph;1");
TF1 *f1 = new TF1("f1",myfunc,-1000,1000,0);

double s1down_1=f1->GetX(y2,-1000,0);
double s1up_1=f1->GetX(y2,0,1000);
double s2down_1=f1->GetX(y1,-1000,0);
double s2up_1=f1->GetX(y1,0,1000);

double min_1=f1->GetMinimumX(-10,10);
...
double s1down_3=f3->GetX(y2,-1000,0);
double s1up_3=f3->GetX(y2,0,1000);
double s2down_3=f3->GetX(y1,-1000,0);
double s2up_3=f3->GetX(y1,0,1000);

double min_3=f3->GetMinimumX(-10,10);
...
char path5[100];
//sprintf(path5,"./fit_v100_m_1T_RunII/%s/LS_objects_k_%s.root",argv[1],argv[1]);                       
sprintf(path5,"./fit_vUL010_m_1T_countings_RunII_/%s/LS_objects_k_%s.root",argv[1],argv[1]);                       
...
double s1down_m_1T=f5->GetX(y2,-1000,0);
double s1up_m_1T=f5->GetX(y2,0,1000);
double s2down_m_1T=f5->GetX(y1,-1000,0);
double s2up_m_1T=f5->GetX(y1,0,1000);

double min_m_1T=f5->GetMinimumX(-10,10);
...
char path6[100];
sprintf(path6,"./fit_v100_PT_j1_RunII/%s/LS_objects_k_%s.root",argv[1],argv[1]);                       
...
char path7[100];
sprintf(path7,"./fit_v100_Eta_l1_RunII/%s/LS_objects_k_%s.root",argv[1],argv[1]);               
...
char path8[100];
sprintf(path8,"./fit_v100_Eta_l2_RunII/%s/LS_objects_k_%s.root",argv[1],argv[1]);                
...
char path9[100];
sprintf(path9,"./fit_v100_Eta_j1_RunII/%s/LS_objects_k_%s.root",argv[1],argv[1]);            
...
char path10[100];
sprintf(path10,"./fit_v100_Eta_j2_RunII/%s/LS_objects_k_%s.root",argv[1],argv[1]);
...
char path11[100];
sprintf(path11,"./fit_v100_PT_j2_RunII/%s/LS_objects_k_%s.root",argv[1],argv[1]);                       
...
char path12[100];
sprintf(path12,"./fit_v100_deltaPhijj_RunII/%s/LS_objects_k_%s.root",argv[1],argv[1]);                       
...
char path13[100];
sprintf(path13,"./fit_v100_M1T_RunII/%s/LS_objects_k_%s.root",argv[1],argv[1]);                       
...
char path14[100];
sprintf(path14,"./fit_v100_M_O1_RunII/%s/LS_objects_k_%s.root",argv[1],argv[1]);                       
...
char path15[100];
sprintf(path15,"./fit_v100_Zeppenfeld_12_RunII/%s/LS_objects_k_%s.root",argv[1],argv[1]);                       
...
char path16[100];
sprintf(path16,"./fit_v100_Zeppenfeld_1_RunII/%s/LS_objects_k_%s.root",argv[1],argv[1]);                       
...
char path17[100];
sprintf(path17,"./fit_v100_Zeppenfeld_2_RunII/%s/LS_objects_k_%s.root",argv[1],argv[1]);                       
...
char path18[100];
sprintf(path18,"./fit_v100_deltaEtajj_RunII/%s/LS_objects_k_%s.root",argv[1],argv[1]);                       
...
//------------Bin1 = DNN+countings------------------
  ...
  Gdown->SetBinContent(2,s1down_0); 
  Gup->SetBinContent(2,s1up_0);
  Ydown->SetBinContent(2,s2down_0);
  Yup->SetBinContent(2,s2up_0);
  char bdttag[10];
  sprintf(bdttag, "%s DNN + countings", argv[1]);
  Yup->GetXaxis()->SetBinLabel(2, bdttag);

//------------Bin2 = DNN+DNN ------------------

  Gdown->SetBinContent(4,s1down_1); 
  Gup->SetBinContent(4,s1up_1);
  Ydown->SetBinContent(4,s2down_1);
  Yup->SetBinContent(4,s2up_1);
  char bdttag2[10];
  sprintf(bdttag2, "%s DNN + %s DNN", argv[1], argv[1]);
  Yup->GetXaxis()->SetBinLabel(4, bdttag2);
...
//------------Bin6 = m_1T------------------
  ...
  Gup->SetBinContent(12,s1up_m_1T);
  Ydown->SetBinContent(12,s2down_m_1T);
  Yup->SetBinContent(12,s2up_m_1T);
  Yup->GetXaxis()->SetBinLabel(12, "M_{1T}");
...
char plotname[30];
sprintf(plotname,"CIplotsUL_RunII/CI_%s.png",argv[1]);        // name of .png file in output
...
else if(strcmp(argv[1],"fS0")==0){
                           a=-38.;
                           b=38.;
                           }
else if(strcmp(argv[1],"fS1")==0){
                           a=-200.;
                           b=200.;
                           }
else if(strcmp(argv[1],"fM0")==0){
                           a=-30.;
                           b=30.;
                           }
else if(strcmp(argv[1],"fM1")==0){
                           a=-40;
                           b=40;
                           }
else if(strcmp(argv[1],"fM6")==0){
                           a=-65.;
                           b=65.;
                           }
else if(strcmp(argv[1],"fM7")==0){
                           a=-100.;
                           b=100.;
                           }
else if(strcmp(argv[1],"fT0")==0)
    { 
    a=-3.;
    b=3.;
    }
...
latexLabel.DrawLatex(0.16, 0.93, "CMS   L = 137.13 fb^{-1}");
...
latexLabelI.DrawLatex(0.20, 0.11, "mjj   mll  3taulep m_o1 pt_j1    pt_j2   #eta_l1    #eta_l2    #eta_j1   #eta_j2     met   #Delta#phi_jj   ");

TLatex latexLabelI;  
latexLabelI.SetNDC(); 
latexLabelI.SetTextSize(0.03);
latexLabelI.SetTextAngle(45);
latexLabelI.DrawLatex(0.185, 0.09, "BDT dim-8");

TLatex latexLabelII;  
latexLabelII.SetNDC(); 
latexLabelII.SetTextSize(0.03);
latexLabelII.SetTextAngle(45);
latexLabelII.DrawLatex(0.23, 0.09, "m_{jj}");
...
cifile.open(tablename, fstream::out);
  ...
  cifile<<"  \\hline"<<endl;
  cifile<<"  DNN + countings "<<argv[1]<<"&"<<"$["<<s1down_0<<","<<s1up_0<<"]$"<<"&"<<"$["<<s2down_0<<","<<s2up_0<<"]$"<<"\\\\"<<endl;
  cifile<<"  DNN"<<"$["<<s1down_1<<","<<s1up_1<<"]$"<<"&"<<"$["<<s2down_1<<","<<s2up_1<<"]$"<<"\\\\"<<endl;
  cifile<<"  $m_{o1}$ + countings&"<<"$["<<s1down_2<<","<<s1up_2<<"]$"<<"&"<<"$["<<s2down_2<<","<<s2up_2<<"]$"<<"\\\\"<<endl;
  cifile<<"  $m_{o1}$&"<<"$["<<s1down_3<<","<<s1up_3<<"]$"<<"&"<<"$["<<s2down_3<<","<<s2up_3<<"]$"<<"\\\\"<<endl;
  /*  
  cifile<<"  $\\Delta\\phi_{jj}$&"<<"$["<<s1down_deltaPhijj<<","<<s1up_deltaPhijj<<"]$"<<"&"<<"$["<<s2down_deltaPhijj<<","<<s2up_deltaPhijj<<"]$"<<"\\\\"<<endl;
  cifile<<"  \\hline"<<endl;
  cifile<<"  $p_T^{l1}$&"<<"$["<<s1down_m_jjtaulep<<","<<s1up_m_jjtaulep<<"]$"<<"&"<<"$["<<s2down_m_jjtaulep<<","<<s2up_m_jjtaulep<<"]$"<<"\\\\"<<endl;
