...
usage = 'python3 PreFitPostFit_v5.py'
...
def PreFitPostFit_v2(region, channel, variable, outdir, years, infold, sb = True, isUL = True, LogX = False):
    ...
    for y in lyears:
        ...
        if not "RunII" in y:
            ...
            totsighistname = region +"_" + y + "_prefit/TotalSig"
            totsbhistname = region +"_" + y + "_prefit/TotalProcs"
        else:
            tothistname = "prefit/TotalBkg"
            totsighistname = "prefit/TotalSig"
            totsbhistname = "prefit/TotalProcs"
 
        h_prefit[yul] = OrderedDict()
        h_prefit[yul]['totalsig'] = f_mlfit.Get(totsighistname)                     ####EDIT    total signal
        if opt.lastbins:
            h_prefit[yul]['totalsig'].GetXaxis().SetRangeUser(firstbin, lastbin)
        if toScale:
            h_prefit[yul]['totalsig'].Scale(1, "width") ###scalewidth 

        h_prefit[yul]['totalb'] = f_mlfit.Get(tothistname)                        ####EDIT    total bkgs 
        if opt.lastbins:
            h_prefit[yul]['totalb'].GetXaxis().SetRangeUser(firstbin, lastbin)
        if toScale:
            h_prefit[yul]['totalb'].Scale(1, "width") ###scalewidth 
        
        h_prefit[yul]['totalsb'] = f_mlfit.Get(totsbhistname)               ####EDIT  total s+b
        if opt.lastbins:
            h_prefit[yul]['totalsb'].GetXaxis().SetRangeUser(firstbin, lastbin)
        if toScale:
            h_prefit[yul]['totalsb'].Scale(1, "width") ###scalewidth 
       
        print "\n\n\nyul:", yul, "\n", "h_prefit[yul]:", h_prefit[yul], "\n\n\n"
        ...
        ##h_minor_prefit = TH1F("h_minor_prefit","h_minor_prefit",len(binLowE)-1,array('d',binLowE))'
        ...
        for process in processes: 
            ...
            if not "RunII" in y:
                ...
                if opt.lastbins:
                    h_prefit[yul][process].GetXaxis().SetRangeUser(firstbin, lastbin)
                if toScale:
                    h_prefit[yul][process].Scale(1, "width") ###scalewidth 
                ...
                for ide, era in enumerate(eras):
                    hera = h_prefit["UL"+era][process]
                    if opt.lastbins:
                        hera.GetXaxis().SetRangeUser(firstbin, lastbin)
                    h_prefit[yul][process].Add(hera)
                #if len(lyears) == 1:
                    #h_prefit[yul][process].Scale(1, "width") ###scalewidth 
        ...
        for process in processes:
            ...
            if not "RunII" in y:
                ...
                if opt.lastbins:
                    h_postfit[yul][process].GetXaxis().SetRangeUser(firstbin, lastbin)
                if toScale:
                    h_postfit[yul][process].Scale(1, "width") ###scalewidth 
                ...
                h_postfit[yul][process] = TH1F(process, var + "_" + channel, totnbins, array('d',binLowE))
                for ide, era in enumerate(eras):
                    hera = h_postfit["UL"+era][process]
                    #if len(lyears) == 1:
                        #hera.Scale(1, "width")
                    if opt.lastbins:
                        hera.GetXaxis().SetRangeUser(firstbin, lastbin)
                    h_postfit[yul][process].Add(hera.Clone(""))#h_postfit["UL"+era][process])
            ...
            h_all_postfit.Add(h_postfit[yul][process].Clone(""))
            #if (not process in mainbkgs[region]):
                #h_other_postfit.Add(h_postfit[yul][process])
            h_stack_postfit.Add(h_postfit[yul][process].Clone(""))
        ...
        if not opt.lastbins:
            if not (blind and "SR" in region):
                maximum = max(h_stack_prefit.GetMaximum(),h_data[yul]["Data"].GetMaximum())
            else:
                maximum = h_stack_prefit.GetMaximum()
        else:
            print range(binLowE.index(firstbin)+1, binLowE.index(firstbin)+2)
            print [h_prefit[yul]['totalb'].GetBinContent(idxb) for idxb in range(binLowE.index(firstbin)+1, binLowE.index(lastbin)+2)]
            maximum = max([h_prefit[yul]['totalb'].GetBinContent(idxb) for idxb in range(binLowE.index(firstbin)+1, binLowE.index(lastbin)+2)])
        print "maximum", maximum
        #print h_stack_prefit.GetMaximum(), h_data[yul]["Data"].GetMaximum()

        if not opt.linscale:
            h_stack_prefit.SetMinimum(0.01)
            pad1.SetLogy()
            h_stack_prefit.SetMaximum(maximum*10000)
        else:
            h_stack_prefit.SetMinimum(0.0001)
            h_stack_prefit.SetMaximum(maximum*2)
        ...
        h_all_prefit.SetLineColor(kAzure-4)
        h_all_prefit.SetLineWidth(2)
        ...
        #if sb is True:
        #print("h_prefit[yul]['totalsig']:", h_prefit[yul]['totalsig'])
        h_prefit[yul]['totalsig'].SetLineColor(sigcolor)
        #h_prefit[yul]['totalsig'].SetLineStyle(1)
        h_prefit[yul]['totalsig'].SetLineWidth(3)
        if opt.lastbins:
            h_prefit[yul]['totalsig'].GetXaxis().SetRangeUser(firstbin, lastbin)
        #### printing signal
        #h_prefit[yul]['totalsig'].Draw("hist same")
        h_prefit[yul]['totalsig'].SetFillColor(sigcolor)
        h_stack_prefit.Add(h_prefit[yul]['totalsig'].Clone(""))


        h_stack_prefit.Draw("hist")
        if opt.lastbins:
            h_stack_prefit.GetHistogram().GetXaxis().SetRangeUser(firstbin, lastbin)
        h_stack_prefit.GetYaxis().SetTitle(ytitle)
        h_stack_prefit.GetYaxis().SetTitleFont(42)
        
        h_stack_prefit.GetXaxis().SetLabelOffset(1.8)
        h_stack_prefit.GetYaxis().SetTitleOffset(1.25)
        h_stack_prefit.GetXaxis().SetLabelSize(0.15)
        h_stack_prefit.GetYaxis().SetLabelSize(0.04)
        h_stack_prefit.GetYaxis().SetTitleSize(0.045)
        h_stack_prefit.SetTitle("")
        ...
        for process in processes:
            ...
            legend.AddEntry(h_prefit[yul][process], proclabel, "f")
        ...
        ###print signal
        legend.AddEntry(h_prefit[yul]['totalsig'], siglabel, "f")
        print "label sig:", sigs, sigs[0], ysigtag, siglabel

        #h_err = h_stack_prefit.GetStack().Last().Clone("h_err")
        h_err = h_prefit[yul]['totalsb'].Clone("h_err")
        ##h_err = h_all_prefit.Clone("h_err")
        #h_err = h_prefit[yul]['totalb'].Clone("h_err")
        #h_err.Add(h_prefit[yul]['totalsig'].Clone(""))
        ...
        for i in range(1,h_all_prefit.GetNbinsX()+1):
            ...
            n_all_post = h_prefit[yul]['totalb'].GetBinContent(i)
        ...
        ratiosys_post = h_prefit[yul]['totalb'].Clone()
        for hbin in range(0,ratiosys_post.GetNbinsX()+1):
            ...
            if (h_prefit[yul]['totalb'].GetBinContent(hbin+1)>0):
                ratiosys_post.SetBinError(hbin+1,h_prefit[yul]['totalb'].GetBinError(hbin+1)/h_prefit[yul]['totalb'].GetBinContent(hbin+1))
            else:
                ratiosys_post.SetBinError(hbin+1,0)

        if not "SR" in region:
            ratiosys_post.GetYaxis().SetRangeUser(0.4, 1.6) #-2.1, 3.1)
        else:    
            ratiosys_post.GetYaxis().SetRangeUser(-0.6, 2.6)
        ...
        if not "SR" in region:
            ratiosys_pre.GetYaxis().SetRangeUser(0.4, 1.6) #-2.1, 3.1)
        else:    
            ratiosys_pre.GetYaxis().SetRangeUser(-0.6, 2.6)
        ...
        g_ratio_pre.SetLineColor(kOrange+7)
        g_ratio_pre.SetMarkerColor(kOrange+7)
        ...
        legend2 = TLegend(0.13,0.46,0.43,0.61, "","brNDC");
        ...
        legend2.AddEntry(ratiosys_pre, "#scale[1.5]{Pre-fit Unc.}", "fple")#ple
        #legend2.AddEntry(ratiosys_post, "#scale[1.5]{Post-fit Unc.}", "fple")#ple
        legend2.Draw("same")

        legend3 = TLegend(0.55,0.97,0.75,0.99,"","brNDC");
        ...
        legend3.AddEntry(g_ratio_post, "#scale[1.5]{Post-fit}", "ple")
        legend3.AddEntry(g_ratio_pre, "#scale[1.5]{Pre-fit}", "ple")#ple
        #legend3.Draw("same")
        ...
        data_pull.Add(h_prefit[yul]['totalb'], -1)
        data_pull.Sumw2()
        ...
        for hbin in range(0, data_pull.GetNbinsX()+1):
            if (h_prefit[yul]['totalb'].GetBinContent(hbin)>0):
                if (math.fabs(h_data[yul]["Data"].GetBinError(hbin)*h_data[yul]["Data"].GetBinError(hbin) - h_prefit[yul]['totalb'].GetBinError(hbin)* h_prefit[yul]['totalb'].GetBinError(hbin)) < 0.001) or (h_prefit[yul]['totalb'].GetBinError(hbin) > h_data[yul]["Data"].GetBinError(hbin) ) or (blind and "SR" in region):
                    data_pull.SetBinContent(hbin,0)
                else:
                    sigmaBILL = math.sqrt(h_data[yul]["Data"].GetBinError(hbin)*h_data[yul]["Data"].GetBinError(hbin) - h_prefit[yul]['totalb'].GetBinError(hbin)*h_prefit[yul]['totalb'].GetBinError(hbin))
                    #sigmaBILL = math.sqrt(h_data[yul]["Data"].GetBinError(hbin)*h_data[yul]["Data"].GetBinError(hbin) + h_prefit[yul]['totalb'].GetBinError(hbin)*h_prefit[yul]['totalb'].GetBinError(hbin))
                    data_pull.SetBinContent(hbin,(h_data[yul]["Data"].GetBinContent(hbin) -  h_prefit[yul]['totalb'].GetBinContent(hbin))/sigmaBILL)
                    #print ("data: ", h_data[yul]["Data"].GetBinContent(hbin)) 
                    #print (" +-, ", h_data[yul]["Data"].GetBinError(hbin))
                    #print ("MC: ", h_prefit[yul]['totalb'].GetBinContent(hbin))
                    #print (" +-, ", h_prefit[yul]['totalb'].GetBinError(hbin))
            ...
            if (hbin > 2):
                ...
                if (abs(data_pull.GetBinContent(hbin)) >= 2):
                    ...
                    print " +- ", h_data[yul]["Data"].GetBinError(hbin)
                    print " BKG yield : ", h_prefit[yul]['totalb'].GetBinContent(hbin) 
                    print " +- ", h_prefit[yul]['totalb'].GetBinError(hbin)
                    print "<<<<<<<<<<<<<<"
        ...
        data_pull.GetYaxis().SetTitle("Pull")#frac{(Data-Pred.)}{#sigma}")
        ...
        for hbin in range(0,data_pull_sig.GetNbinsX()+1):
            if (h_prefit[yul]['totalb'].GetBinContent(hbin)>0):
                #print "bin",hbin,"data pull diff", data_pull_sig.GetBinContent(hbin), "sys", h_prefit[yul]['totalb'].GetBinError(hbin)
                data_pull_sig.SetBinContent(hbin, data_pull_sig.GetBinContent(hbin)/h_prefit[yul]['totalb'].GetBinError(hbin))
...
for var in plotvars:
    print "\n\nProcessing prefit for " + var + "..."
    infolder = infile + "/" + var + "/" + modd + "/"
    for ch in channels:
        print "\nProcessing prefit for " + ch + "..."
