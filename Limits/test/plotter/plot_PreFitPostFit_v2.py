...
def plotPreFitPostFit_v2(region,category,fitdiag_file,outdir,lumi,year,sb=False):
  #ELEorMUON 

  datalab = {"dy2016prefit":"DYcr_Y2016_DYcr_prefit", "top2016prefit":"TOPcr_Y2016_TOPcr_prefit", "sr2016prefit":"SR_Y2016combined_signal_region_prefit"} #i dati sono sempre gli stessi, pre e post-fit
 ## potrebbe essere generalizzato con "nested dictionaries"

  f_mlfit = TFile(fitdiag_file,'READ') ###file dei MC
  ...
  # else:
  #h_data = gDirectory.Get(datalab[region])  ### : here we have to specify the path of the TH1F with data. 
  h_data = f_mlfit.Get(datalab[region]+"/data_obs")  ### : here we have to specify the path of the TH1F with data. 
...
#  h_prefit['total'] = f_mlfit.Get("shapes_prefit/"+channel[region]+"/total")     
  h_prefit['total'] = f_mlfit.Get("shapes_prefit/"+channel[region]+"/TotalBkg")                       ####### EDIT. PATH TOTALE BACKGROUNDS DELLA CATEGORIA CHE SI VUOLE PLOTTARE
  ...
  for process in processes: 
    h_prefit[process] = f_mlfit.Get("shapes_prefit/"+channel[region]+"/"+process)                     ######EDIT: per ogni processo in "process" apre la shape CARICA L'HISTO 
  ...
  # Post-Fit
  h_postfit = {}
  h_postfit['totalsig'] = f_mlfit.Get("shapes_fit_b/"+channel[region]+"/total")                     ####EDIT    total signal 
  h_postfit['total'] = f_mlfit.Get("shapes_fit_b/"+channel[region]+"/total")                        ####EDIT    total bkgs 
  ...
  h_postfit['totalv2'] = f_mlfit.Get("shapes_fit_b/"+channel[region]+"/total_background")               ####EDIT  questo è un altro "total" che potrebbe essere s+b
  ...
  for process in processes:
    h_postfit[process] = f_mlfit.Get("shapes_fit_b/"+channel[region]+"/"+process)                       ####EDIT  qui post-fit histo per ogni processo in esame
    ...
    if region in 'signal':                            
      if process is 'gjets' or process is 'zll':                                                  ####EDIT
  ...
  if region is "signal" is "monojet":        ## EDITING NOT NEEDED
    categoryLabel.DrawLatex(0.200,0.80,"monojet");
    categoryLabel.Draw("same");
  elif region is "signal" is "monov":
  ...
  if region in 'signal':
      dummy3.GetXaxis().SetTitle("E_{T}^{miss} [GeV]"  if 'mono' in region else "M_{llJ} [GeV]")
  else:
    dummy3.GetXaxis().SetTitle("Recoil [GeV]" if 'mono' in region else "M_{llJ} [GeV]")
