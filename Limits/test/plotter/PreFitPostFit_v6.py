...
usage = 'python3 PreFitPostFit_v6.py'
...
def PreFitPostFit_v6(region, channel, variable, outdir, years, infold, sb = True, isUL = True, LogX = False):
    ...
    for y in lyears:
        ...
        for process in processes: 
            ...
            color = None 
            subgroup = None
            #print("\n\n\n\ncolor before", color, yproctag)
            for plotsam in plot_list:
                #print("plotsam.label", plotsam.label, yproctag)
            ...
            h_prefit[yul][process].SetLineColor(int(color))
            h_prefit[yul][process].SetFillColor(int(color))
        ...
        for process in processes:
            ...
            #print("process", process, yproctag)

            IsAComp = False
            SampGroup = process
            hTitle = None
            for plotsam in plot_list:
                if not (plotsam.label.startswith("WpWpJJ_") or plotsam.label.startswith("VBS_SSWW_") or plotsam.label.startswith("Others_") or plotsam.label.startswith("Fake") or plotsam.label.startswith("Data") or plotsam.label.startswith("TTTo2L2Nu_") or plotsam.label.startswith("WrongSign_")):
                    continue
                #print "plotsam", plotsam.label
                if hasattr(plotsam, "components") and not (any(plotsam.label.split("_")[0] in sub.label for sub in plotsam.components) or process.startswith("WrongSign") or process.startswith("Fake")):

                    for subplotsam in plotsam.components:
                        if subplotsam.label == yproctag:
                            #print("COP", subplotsam.label, yproctag, plotsam.label)
                            IsAComp = True
                            SampGroup = plotsam.label.split("_")[0]
                            color = plotsam.color
                            hTitle = plotsam.leglabel
                            if SampGroup.startswith("Others"):
                                others_num += 1
                            break
                else:
                    if plotsam.label == yproctag:
            ...
            print(process, IsAComp, SampGroup, color)
            if not "RunII" in y:
                #print("\n\n\nHELLO\n\n\n")
                if not IsAComp:
                    try:
                    ...
                    if not SampGroup in h_postfit[yul]:
                        h_postfit[yul][SampGroup] = TH1F(SampGroup, var + "_" + channel, totnbins, array('d',binLowE))
                        if opt.lastbins:
                            h_postfit[yul][SampGroup].GetXaxis().SetRangeUser(firstbin, lastbin)
                    try:
                        f_mlfit.Get(histname).GetEntries()
                    except:
                        pass
                    else:
                        temph = f_mlfit.Get(histname)
                        if opt.lastbins:
                            temph.GetXaxis().SetRangeUser(firstbin, lastbin)
                        if toScale:
                            #h_postfit[yul][process].Scale(1, "width") ###scalewidth 
                            temph.Scale(1, "width") ###scalewidth 
                        h_postfit[yul][SampGroup].Add(temph.Clone(""))
                #for i in range(1,totnbins+1):
                ...
                h_postfit[yul][SampGroup] = TH1F(SampGroup, var + "_" + channel, totnbins, array('d',binLowE))
                for ide, era in enumerate(eras):
                    hera = h_postfit["UL"+era][SampGroup]
                    #if len(lyears) == 1:
                        #hera.Scale(1, "width")
                    if opt.lastbins:
                        hera.GetXaxis().SetRangeUser(firstbin, lastbin)
                    h_postfit[yul][SampGroup].Add(hera.Clone(""))#h_postfit["UL"+era][SampGroup])
            ...
            h_postfit[yul][SampGroup].SetName(SampGroup + "_" + region + "_" + y + "_postfit")
            h_postfit[yul][SampGroup].SetLineColor(color)
            h_postfit[yul][SampGroup].SetFillColor(color)
            h_postfit[yul][SampGroup].SetTitle(hTitle)
            
            h_all_postfit.Add(h_postfit[yul][SampGroup].Clone(""))
            #if (not SampGroup in mainbkgs[region]):
                #h_other_postfit.Add(h_postfit[yul][SampGroup])
            if SampGroup.startswith("Others"):
                if others_num == 5:
                    h_stack_postfit.Add(h_postfit[yul][SampGroup].Clone(""))
                else:
                    continue
            else:
                h_stack_postfit.Add(h_postfit[yul][SampGroup].Clone(""))
            #print "h_postfit", yul, SampGroup, ":", h_postfit[yul][process].Integral()
        ...
        pad1= ROOT.TPad("pad1", "pad1", 0, 0.372, 1, 1)
        ...
        if not opt.lastbins:
            ...
            #print(list(range(binLowE.index(firstbin)+1, binLowE.index(firstbin)+2)))
        ...
        if not opt.linscale:
            h_stack_postfit.SetMinimum(0.5)
            pad1.SetLogy()
            h_stack_postfit.SetMaximum(maximum*2000)
        else:
        ...
        h_stack_postfit.GetYaxis().SetTitleOffset(0.85)
        h_stack_postfit.GetYaxis().SetLabelSize(0.06)
        h_stack_postfit.GetYaxis().SetTitleSize(0.06)

        latexy = ROOT.TLatex()
        latexy.SetTextFont(42)
        latexy.SetTextSize(0.06)
        #latexy.SetTextAlign(-30)  # Align right
        latexy.SetTextAngle(90)  # Vertical text
        #latexy.DrawLatex(-0.04, 100000, ytitle)  # Adjust the x, y positions as needed
        ...
        legend = TLegend(0.33,0.615,0.97,0.915)
        ...
        for histt in list(h_postfit[yul].values()):
            #print "histt", histt, histt.GetTitle(), histt.GetName()
            if not histt.GetName().startswith("Total"):
                legend.AddEntry(histt, histt.GetTitle() , "f")
        ...
        pad2 = ROOT.TPad("pad2", "pad2", 0, 0.229, 1, 0.37)
        ...
        ratiosys_post.GetYaxis().SetLabelSize(0.2)
        ratiosys_post.GetYaxis().SetTitleSize(0.2)
        ...
        ratiosys_pre.GetYaxis().SetNdivisions(1003)
        ratiosys_pre.GetYaxis().SetLabelFont(42)
        ratiosys_pre.GetYaxis().SetTitleFont(42)
        ratiosys_pre.GetYaxis().SetTitleOffset(0.17)
        ratiosys_pre.GetYaxis().SetLabelSize(0.25)
        ratiosys_pre.GetYaxis().SetTitleSize(0.23)
        ratiosys_pre.GetYaxis().CenterTitle(1)
        ...
        ratio.GetYaxis().SetTitleSize(0.3)
        ...
        legend2 = TLegend(0.12,0.02,0.62,0.28, "","brNDC");
        ...
        legend2.SetTextSize(0.2)
        legend2.AddEntry(ratiosys_pre, "#scale[1]{Pre-fit Unc.}")#, "fple")#ple
        legend2.AddEntry(ratiosys_post, "#scale[1]{Post-fit Unc.}")#, "fple")#ple
        legend2.Draw("same")
        ...
        pad3= ROOT.TPad("pad3", "pad3", 0, 0 , 1, 0.214) #0.205)
        ...
        data_pull.GetYaxis().SetTitleOffset(0.25)
        data_pull.GetYaxis().SetLabelSize(0.19)
        data_pull.GetYaxis().SetTitleSize(0.16)
        ...
        data_pull.GetXaxis().SetTitleOffset(1)
        data_pull.GetXaxis().SetLabelSize(0.18)
        data_pull.GetXaxis().SetLabelOffset(0.02)
        data_pull.GetXaxis().SetTitleSize(0.18)
...
for var in plotvars:
    ...
    for ch in channels:
        ...
        PreFitPostFit_v6(ch, "ltau", var, indir, years, infolder)
