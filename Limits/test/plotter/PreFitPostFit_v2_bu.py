...
parser.add_option('--tagfolder', dest='tagfold', type='string', default = '', help = 'Folder to plot')
...
setname = "Stat.Limits.settings_" + opt.tag
if opt.wfc:
    setname += "_WithFakeCR"
if opt.pdfttdy:
...
#infile = 'postfit_dev_' + pdftype + "_DYinOS_oneIFSR"
infile = 'postfit' + pdftype + "_DYinOS_oneIFSR"
if not DYrp:
    infile += 'nodyrp_'
infile += indir + '/' + sr_var + '_' + cr_var + '_' + opt.era

if opt.wfc:
    infile += "_WithFakeCR"
if opt.pdfttdy:
    infile += "_PDFWithTTDY"
infile += "/" + opt.tagfold.replace("_", "") + "/" + srvars + "/" + modd + "/histo_" + srvars + "_"
...
sfolder += opt.tagfold + "/"
#prefolder = sfolder + "prefit_" + modd + "_" + srvar + "_" + crvar
postfolder = sfolder + "postfit_" + pdftype + "/" + modd
if not DYrp:
    postfolder += '_nodyrp_'
if opt.wfc:
    postfolder += "_WithFakeCR"
if opt.pdfttdy:
    postfolder += "_PDFWithTTDY"
if opt.lastbins:
    postfolder += "_lastbins"
    ...
    for y in lyears:
        ...
        for hbin in range(0,ratiosys_post.GetNbinsX()+1):
            ...
            if (h_all_postfit.GetBinContent(hbin+1)>0):
                ratiosys_post.SetBinError(hbin+1,h_all_postfit.GetBinError(hbin+1)/h_all_postfit.GetBinContent(hbin+1))
            else:
        ...
        for hbin in range(0, data_pull.GetNbinsX()+1):
            if (h_all_postfit.GetBinContent(hbin)>0):
                if (math.fabs(h_data[yul]["Data"].GetBinError(hbin)*h_data[yul]["Data"].GetBinError(hbin) - h_all_postfit.GetBinError(hbin)* h_all_postfit.GetBinError(hbin)) < 0.001) or (h_all_postfit.GetBinError(hbin) > h_data[yul]["Data"].GetBinError(hbin) ) or "SR" in region:
                    data_pull.SetBinContent(hbin,0)
                else:
                    sigmaBILL = math.sqrt(h_data[yul]["Data"].GetBinError(hbin)*h_data[yul]["Data"].GetBinError(hbin) - h_all_postfit.GetBinError(hbin)*h_all_postfit.GetBinError(hbin))
                    #sigmaBILL = math.sqrt(h_data[yul]["Data"].GetBinError(hbin)*h_data[yul]["Data"].GetBinError(hbin) + h_all_postfit.GetBinError(hbin)*h_all_postfit.GetBinError(hbin))
                    data_pull.SetBinContent(hbin,(h_data[yul]["Data"].GetBinContent(hbin) -  h_all_postfit.GetBinContent(hbin))/sigmaBILL)
                    #print ("data: ", h_data[yul]["Data"].GetBinContent(hbin)) 
                    #print (" +-, ", h_data[yul]["Data"].GetBinError(hbin))
                    #print ("MC: ", h_all_postfit.GetBinContent(hbin))
                    #print (" +-, ", h_all_postfit.GetBinError(hbin))
            ...
            if (hbin > 2):
                ...
                if (abs(data_pull.GetBinContent(hbin)) >= 2):
                    ...
                    print " +- ", h_data[yul]["Data"].GetBinError(hbin)
                    print " BKG yield : ", h_all_postfit.GetBinContent(hbin) 
                    print " +- ", h_all_postfit.GetBinError(hbin)
                    print "<<<<<<<<<<<<<<"
        ...
        for hbin in range(0,data_pull_sig.GetNbinsX()+1):
            if (h_all_postfit.GetBinContent(hbin)>0):
                #print "bin",hbin,"data pull diff", data_pull_sig.GetBinContent(hbin), "sys", h_all_postfit.GetBinError(hbin)
                data_pull_sig.SetBinContent(hbin, data_pull_sig.GetBinContent(hbin)/h_all_postfit.GetBinError(hbin))
        ...
        outsave = postfolder+"/"+variable+"_"+region+"_" + yul
        if not opt.linscale:
            outsave += "_logscale"
        else:
            outsave += "_linscale"
