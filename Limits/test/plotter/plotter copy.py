...
def plotter(flavor,fitdiag_file,region,lumi,outdir="./output",year=161718,sb=True,asymbins=True,LogX=False):
    ...
    mainbkgs = {
            "SR_Y2017combined_signal_region":["TTtW","DY","Other"],
            "DYcr_Y2017_DYcr":["TTtW","DY","Other"],
            "TOPcr_Y2017_TOPcr":["TTtW","DY","Other"],
            }
    processes = [
        'Other', 
        'TTtW',
    ...
    binLowE = []
    binLowE2 = []
    MllJBin=[0.,0.2,0.4,0.6,0.8,1.,1.4,2.,10.] #   MllJBin=[0.,200.,400.,600.,800.,1000.,1400.,2000.,10000.]
    ...
    if asymbins is True:
      ...
      for process in processes: 
        ...
        h_stack_prefit = THStack("h_stack_prefit","h_stack_prefit")
        h_all_prefit2 = TH1F("h_all_prefit2","h_all_prefit2",len(binLowE2)-1,array('d',binLowE2))  ###rebinned
        h_prefit2[process] = TH1F("","",len(binLowE2)-1,array('d',binLowE2))      #rebinned
        for process in processes: 
          h_prefit[process] = f_mlfit.Get(region+"_prefit/"+process)
          if (not h_prefit[process]): continue
          if (str(h_prefit[process].Integral())=="nan"): continue
          for i in range(1,h_prefit[process].GetNbinsX()+1):
              ...
              width = h_prefit[process].GetBinLowEdge(i+1)-h_prefit[process].GetBinLowEdge(i)
              h_prefit[process].SetBinContent(i,content) #*width
              h_prefit2[process].SetBinContent(i,content) #*width       #rebinned
          h_prefit[process].SetLineColor(colors[process])
          h_prefit[process].SetFillColor(colors[process])
          h_prefit2[process].SetLineColor(colors[process])
          h_prefit2[process].SetFillColor(colors[process])
          h_all_prefit.Add(h_prefit[process])
          h_all_prefit2.Add(h_prefit2[process])
          if (not process in mainbkgs[region]):
              h_other_prefit.Add(h_prefit[process])
          h_stack_prefit.Add(h_prefit[process])  
    ...
    for process in processes:
        ...
        h_postfit2[process] = TH1F("","",len(binLowE2)-1,array('d',binLowE2))
        ...
        for i in range(1,h_postfit[process].GetNbinsX()+1):
          ...
          h_postfit2[process].SetBinContent(i,content)
        h_postfit[process].SetLineColor(1)
        h_postfit[process].SetFillColor(colors[process])
        ##
        h_postfit2[process].SetLineColor(1)
        h_postfit2[process].SetFillColor(colors[process])
        ##
        ...
        if asymbins is True:
            h_stack_postfit.Add(h_postfit2[process])
        else:
...
#########################################################
    ...
    if asymbins is True:
      dummy = h_all_prefit2.Clone("dummy")
    else:
...
#    h_postfit['totalsig'].SetFillStyle(3144);
    if sb:
      if asymbins is True:
        h_postfit2['totalsig'].Draw("samehist")
      else:
    ...
    legend.AddEntry(h_postfit['DY'], "Drell-Yann", "f")
    legend.AddEntry(h_postfit['TTtW'], "Top", "f")
    legend.AddEntry(h_postfit['Other'], "Other", "f")
    if sb:
      if asymbins is True:
        legend.AddEntry(h_postfit2['totalsig'], "SIGNAL+B POST-FIT", "l")           ###### qui conviene mettere SOLO il segnale 
      else:
        legend.AddEntry(h_postfit['totalsig'], "SIGNAL+B POST-FIT", "l")           ###### qui conviene mettere SOLO il segnale 
#    else:
...
#        legend.AddEntry(h_other_prefit, "Other Backgrounds", "f")
    ...
    if region is "signal" is "monojet":
    ...
    if region is "signal":                                          ## questo andrà modificato per mettere un label per ogni regione
      categoryLabel.DrawLatex(0.200,0.80,"monojet");
      categoryLabel.Draw("same");
    elif region is "signal":
    ...
    if flavor is "electron":
      ...
      dummy3.GetXaxis().SetTitle("M(#mu #mu J) [TeV]")
    else:
      dummy3.GetXaxis().SetTitle("M(e #mu J) [TeV]")
...
### save plots and close
    ...
    if not os.path.exists(outdir):
      os.makedirs(outdir)
    c.SaveAs(outdir+region+"_"+flavor+".pdf")
    c.SaveAs(outdir+region+"_"+flavor+".png")
    c.SaveAs(outdir+region+"_"+flavor+".C")
    c.SaveAs(outdir+region+"_"+flavor+".root")
    c.Close()
    f_mlfit.Close()
...
plotter("electron","./inputs/PostFit_histograms.root","SR_Y2017combined_signal_region",138,"/eos/user/m/mpresill/www/HN/approval_plots/")
#plotter("./inputs/PostFit_histograms.root","DYcr_Y2017_DYcr_prefit")
#plotter("./inputs/PostFit_histograms.root","TOPcr_Y2017_TOPcr_prefit")